...
function secondary_destinations = Blend_waste_HYD(blend_progress_cells,SpecLimits,petro_props)
global PVPs VP_MWs
secondary_blending = [];
...
for i=1:size(blend_progress_cells,1)
    ...
    elseif sum(secondary_mix(:,3)) == 0 % If there's nothing in the mixture, don't mix it lol
        ...
        HC_secondary_mix = HYD_alc(secondary_mix);
        secondary_props0 = HCmix(HC_secondary_mix,blendstockVP_subset);
...
%         secondary_meetspecs(3,:) = checkspecs(secondary_props_gasoline,SpecLimits(:,:,3));
        ...
        if sum(secondary_meetspecs(1,:)) == nspec
            ...
            [diesel_blend_max2, dmix_props2, dFailedProp2, dHiLo2] = Fuel_Blender(secondary_props_diesel, petro_props(1,:), SpecLimits(:,:,1), 0, "hydrocarbon", "Diesel", HC_secondary_mix);
        ...
        if sum(secondary_meetspecs(2,:)) == nspec
            ...
            [jet_blend_max2, jmix_props2, jFailedProp2, jHiLo2] = Fuel_Blender(secondary_props_jet, petro_props(2,:), SpecLimits(:,:,2), 0, "hydrocarbon", "Jet", HC_secondary_mix);
        ...
        if sum(secondary_meetspecs(3,:)) == nspec
            ...
            [auto_blend_max2, amix_props2, aFailedProp2, aHiLo2] = Fuel_Blender(secondary_props_gasoline, petro_props(3,:), SpecLimits(:,:,3), 0, "hydrocarbon", "Gasoline", HC_secondary_mix);
